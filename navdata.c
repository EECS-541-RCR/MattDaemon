...
/* Private variables */
static pthread_t nav_thread = 0; // CHANGE: was 1
...
static void* navdata_loop(void *arg) {
  ...
  while ( nav_thread_alive ) {
    ...
    size = recvfrom (navdata_udp_socket, &msg[0], NAVDATA_BUFFER_SIZE, 0, (struct sockaddr *)my_addr, (socklen_t *)&addr_in_size);
    if (size == -1) {
      if (prevErrno != errno) {
        printf("Errno: %s\n", strerror(errno));
        prevErrno = errno;
